...
//        루트 댓글 저장 로직
    ...
    public Long createReplyComment(CreateReplyDto createReplyDto, String userId, String userNickName){
        Board board = boardRepository.findById(createReplyDto.getBoardId())
                .orElseThrow(() -> new NotFound("존재하지 않는 게시글입니다."));
        if(board.isDeleted()){
            throw new BadParameter("이미 삭제된 게시글입니다.");
        }
        ...
        Comment reply = createReplyDto.toEntity(createReplyDto.getReplyBody(), userId, userNickName, board, comment);
        try{
